...
def fvgs(bars: list[Bar], min_size: float = 0.0, tick: float | str | None = None):
    ...
    for i in range(1, len(bars) - 1):
        ...
        # VII liegt vor, wenn sich die KOERPER (Open/Close) zweier benachbarter Kerzen NICHT
        # ueberlappen (die Luecke zwischen Close der einen und Open der naechsten ist dann nicht
        # durch einen Koerper aufgefuellt). Dann bildet der aeussere Close/Open-Rand die FVG-Grenze;
        # ueberlappen die Koerper, gilt der Wick. Verifiziert an realen MNQ-Kerzen 2026-08-13
        # (00:09/00:12/00:13/00:14). Siehe wiki/concepts/Volume Imbalance (VII).md.
        def _vii(x, y):  # Koerper disjunkt?
            return max(x.o, x.c) < min(y.o, y.c) or max(y.o, y.c) < min(x.o, x.c)
        if c.l > a.h and c.l - a.h > min_size:
            side = "bullish"
            lo = min(a.c, m.o) if _vii(a, m) else a.h
            hi = max(m.c, c.o) if _vii(m, c) else c.l
        elif c.h < a.l and a.l - c.h > min_size:
            side = "bearish"
            hi = max(a.c, m.o) if _vii(a, m) else a.l
            lo = min(m.c, c.o) if _vii(m, c) else c.h
        else:
            continue
...
def fvg_selfcheck() -> None:
    """Regression fuer die VII-inklusive FVG-Messung. Kernregel: eine VII (Close/Open-Rand statt
    Wick) liegt vor, wenn sich die KOERPER (Open/Close) der beiden Nachbarkerzen NICHT ueberlappen;
    sonst gilt der Wick. Verankert an vier realen, von Hand geprueften MNQ-Kerzen-Tripeln
    (2026-08-13), zusaetzlich ein Wick-Fallback ohne VII. Wird von algo/selfcheck.py aufgerufen."""
    from datetime import datetime as _dt
    def mk(hh, o, h, l, c):
        return Bar(_dt(2026, 8, 13, 0, hh, tzinfo=NY), o, h, l, c)
    # (Zeit, O, H, L, C) der 1m-Kerzen 00:08-00:15 vom 2026-08-13.
    bars = {
        8:  mk(8,  29887.25, 29887.25, 29884.25, 29885.25),
        9:  mk(9,  29885.50, 29889.25, 29885.25, 29889.00),
        10: mk(10, 29889.50, 29892.25, 29887.75, 29889.50),
        11: mk(11, 29890.50, 29890.75, 29887.50, 29888.25),
        12: mk(12, 29888.00, 29890.00, 29887.50, 29887.50),
        13: mk(13, 29887.25, 29887.25, 29876.75, 29877.50),
        14: mk(14, 29878.00, 29879.50, 29869.00, 29870.00),
        15: mk(15, 29870.00, 29872.50, 29868.75, 29872.25),
    }
    # (Displacement-Minute) -> erwartete (side, lo, hi), von Jannes' Chart-Screenshots abgelesen.
    expect = {
        9:  ("bullish", 29887.25, 29889.50),  # Bottom Wick a.high, Top VII c.open
        12: ("bearish", 29887.25, 29888.25),  # beide Koerper disjunkt -> beide VII
        13: ("bearish", 29879.50, 29887.50),  # Top VII a.close, Bottom Wick c.high (Koerper ueberlappen)
        14: ("bearish", 29872.50, 29876.75),  # beide Koerper ueberlappen -> beide Wicks
    }
    for mid, (side, lo, hi) in expect.items():
        f = fvgs([bars[mid - 1], bars[mid], bars[mid + 1]])[0]
        assert f["side"] == side and f["lo"] == lo and f["hi"] == hi, (mid, f)
    ...
    print("fvg_selfcheck: VII-Messung (Koerper-Disjunkt-Regel) OK")
