---
tags: [source, ict, trading-ict, curriculum]
created: 2026-08-01
updated: 2026-08-06
raw: "[[Core Content 2016]]"
raw_path: "raw/trading-ict/Core Content/Core Content 2016.md"
---

# Core Content 2016 (Source)

Wurzel des ICT-Core-Content-2016-Curriculums — kein eigener Inhalt, reine Inhaltsübersicht über die
zehn Monats-Seiten. Alle 60 Lectures des Kurses sind ingested und tragen ein `curriculum:`-Feld,
das auf ihre Monats-Seite zurückverweist.

| Monat | Thema | Lectures |
|---|---|---|
| [[Month 1 (Source)]] | Marktmechanik-Grundlagen | 5 |
| [[Month 02 (Source)]] | Risiko-/Money-Management, Setup-Selektion | 8 |
| [[Month 03 (Source)]] | Institutioneller Order Flow | 5 |
| [[Month 04 (Source)]] | Order-Block-Vertiefung | 6 |
| [[Month 05 (Source)]] | IPDA, Open Float, Intermarket | 9 |
| [[Month 06 - Swing Trading Model (Source)]] | Swing-Trading-Modell | 4 |
| [[Month 07 - Weekly (Source)]] | Weekly Range | 8 |
| [[Month 08 - Daytrades (Source)]] | Daytrading | 8 |
| [[Month 9 - Scalping-Daytrades (Source)]] | Scalping & Daytrades | 8 |
| [[Month 10 (Source)]] | Futures-Spezifika & Opening Range | 4 |
| [[Month 11 (Source)]] | Mega-Trades | 3 |

> ✅ **Lücke geschlossen (2026-08-06)**: ein „Month 02" existiert im Notion-Export nicht — die
> Sprungliste sprang ursprünglich von Monat 1 direkt auf Monat 3. Inhaltlich nachgereicht über
> 8 YouTube-Videos derselben Mentorship (Core Content Month 02, Aug. 2022), ingested unter
> `raw/trading-ict/2026/yt-*-transcript.md`. Eingeordnet wie jeder andere Monat über die neue
> Curriculum-Sprungseite [[Month 02 (Source)]].

## Lücken im Rohmaterial

- `Reeinforced Liquidity Pools - When to anticipate Raids` (Monat 4) — Rohdatei leer (0 Byte).
- `Basics & Opening Range Concept` (Monat 10) — nur Bild-Embeds, kein Text.
- `How Market Makers Condition The Market` (Monat 1) — nur Überschrift + ein Bild.
