...
## Task 10: Protokoll- und Datenartefakte + Domänenkontext algo + graphify

**Files:**
- Modify: `CLAUDE.2.1.md` (Zeilen ~386–423 der Baseline)

**Hinweis:** Der `graphify`-Abschnitt ist im Original auf Englisch. Da Task 2 bereits die
"immer Deutsch antworten"-Regel einfuehrt und alle uebrigen 16 Abschnitte auf Deutsch stehen,
wird dieser Abschnitt zur Konsistenz mit uebersetzt (inhaltlich unveraendert) — das ist eine
bewusste, kleine Erweiterung ueber reine Umformulierung hinaus und wird im Abgabebericht als
solche benannt.
...
Dieses Projekt hat einen Wissensgraphen unter `graphify-out/` mit God Nodes, Community-Struktur
und dateiübergreifenden Beziehungen.

Regeln:
- Führe bei Codebase-Fragen zuerst `graphify query "<Frage>"` aus, wenn `graphify-out/graph.json`
  existiert. Nutze `graphify path "<A>" "<B>"` für Beziehungen und `graphify explain "<Konzept>"`
  für fokussierte Konzepte. Diese liefern einen begrenzten Teilgraphen, meist deutlich kleiner
  als `GRAPH_REPORT.md` oder rohe Grep-Ausgabe.
- Nutze für breite Navigation `wiki/index.md` (den kuratierten Katalog) statt rohes
  Quellcode-Browsing.
- Lies `graphify-out/GRAPH_REPORT.md` nur für eine breite Architektur-Review oder wenn
  query/path/explain nicht genug Kontext liefern.
- Führe nach Code-Änderungen `graphify update .` aus, um den Graphen aktuell zu halten
  (nur AST, keine API-Kosten).
```
...
- [ ] graphify-Regeln inhaltlich alle 4 Punkte vollstaendig, nur Sprache DE statt EN

- [ ] **Step 3: Commit**

```bash
git add CLAUDE.2.1.md
git commit -m "docs(claude-2.1): Protokoll-/Datenartefakte, Domaenenkontext algo und graphify umformuliert/uebersetzt"
```
...
umformuliert wurde (leicht/mittel/stark), und nenne explizit die eine bewusste
Inhaltserweiterung (graphify-Abschnitt EN → DE uebersetzt, Task 10). Bestaetige, dass Step 2
(Struktur-Diff) leer war.
